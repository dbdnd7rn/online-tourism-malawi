...
import { adminResources, adminRoles, resourceEntries, workflowEntries, workflowResources } from './adminConfig'
import './admin.css'
...
function AdminOverview() {
  ...
  const contentTotal = ['heritage', 'museums', 'performances', 'events', 'podcasts', 'creatives'].reduce((total, key) => total + (data.counts[key] || 0), 0)
  const stats = [
    { label: 'Published stories', value: contentTotal, change: 'Across six collections', icon: BookOpen, tone: 'gold' },
    ...
    <div className="admin-dashboard">
      ...
      <section className="admin-dashboard-grid">
        <article className="admin-panel admin-collection-health">
          <div className="admin-panel__head"><div><small>Collection health</small><h2>Six stories, one Malawi.</h2></div><Link to="/admin/content/heritage">Manage content <ArrowRight size={16} /></Link></div>
...
function AdminResourcePage() {
  ...
  const filtered = useMemo(() => items.filter((item) => {
    ...
    const visibilityMatch = visibility === 'all' || (visibility === 'published' ? item.published : !item.published)
  ...
  const togglePublished = async (item) => {
    try {
      const saved = await saveAdminRecord(resource, { ...item, published: !item.published })
      setItems((current) => current.map((entry) => entry.id === saved.id ? saved : entry))
      setToast(saved.published ? 'Published to the website' : 'Moved back to draft')
    } catch (reason) { setError(reason.message || 'Unable to update visibility') }
  }
  const remove = async () => {
    try {
      await deleteAdminRecord(resource, deleteCandidate.id)
      setItems((current) => current.filter((item) => item.id !== deleteCandidate.id))
      setDeleteCandidate(null)
      setToast(`${definition.singular} deleted`)
    } catch (reason) { setError(reason.message || 'Unable to delete record') }
  }

  return (
    <div className="admin-resource-page">
      <AdminPageHeader eyebrow="Content collection" title={definition.label} copy={definition.description} actions={<button className="admin-primary-action" onClick={() => setEditing({ published: true, sort_order: items.length + 1 })}><Plus size={17} /> New {definition.singular}</button>} />
      <section className="admin-collection-summary"><span><Icon size={24} /></span><div><strong>{items.length}</strong><small>Total records</small></div><div><strong>{items.filter((item) => item.published).length}</strong><small>Published</small></div><div><strong>{items.filter((item) => !item.published).length}</strong><small>Drafts</small></div><i /></section>
      <section className="admin-resource-panel">
        <div className="admin-resource-toolbar"><label><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`Search ${definition.shortLabel.toLowerCase()}...`} /></label><div><button className={visibility === 'all' ? 'active' : ''} onClick={() => setVisibility('all')}>All</button><button className={visibility === 'published' ? 'active' : ''} onClick={() => setVisibility('published')}>Published</button><button className={visibility === 'draft' ? 'active' : ''} onClick={() => setVisibility('draft')}>Drafts</button></div><button className="admin-icon-button" onClick={load} aria-label="Refresh collection"><RefreshCw size={17} /></button></div>
        {error && <AdminNotice icon={Bell} title="Something needs attention" copy={error} action={<button onClick={load}>Try again</button>} />}
        {loading ? <AdminLoading label={`Loading ${definition.shortLabel.toLowerCase()}`} /> : filtered.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Record</th>{definition.columns.slice(1).map((column) => <th key={column.key}>{column.label}</th>)}<th>Visibility</th><th><span className="sr-only">Actions</span></th></tr></thead><tbody>{filtered.map((item) => <tr key={item.id}><td><div className="admin-record-title">{item.image ? <img src={item.image} alt="" /> : <span><Image size={18} /></span>}<div><strong>{item[definition.titleField]}</strong><small>/{item.slug || item.id}</small></div></div></td>{definition.columns.slice(1).map((column) => <td key={column.key}>{column.type === 'date' ? formatDate(item[column.key]) : column.type === 'boolean' ? item[column.key] ? <span className="admin-verified"><BadgeCheck size={15} /> Yes</span> : 'No' : item[column.key] || '—'}</td>)}<td><button className={`admin-publish-toggle ${item.published ? 'active' : ''}`} onClick={() => togglePublished(item)} aria-label={`${item.published ? 'Unpublish' : 'Publish'} ${item[definition.titleField]}`}><i /><span>{item.published ? 'Published' : 'Draft'}</span></button></td><td><div className="admin-row-actions"><button onClick={() => setEditing(item)} aria-label={`Edit ${item[definition.titleField]}`}><Pencil size={16} /></button><Link to={definition.publicPath(item)} target="_blank" rel="noreferrer" aria-label={`View ${item[definition.titleField]} on the website`}><ExternalLink size={15} /></Link><button className="danger" onClick={() => setDeleteCandidate(item)} aria-label={`Delete ${item[definition.titleField]}`}><Trash2 size={15} /></button></div></td></tr>)}</tbody></table></div> : <AdminEmpty icon={Icon} title="No matching records" copy="Adjust your search or create the first record in this view." action={<button className="admin-primary-action" onClick={() => setEditing({ published: true, sort_order: items.length + 1 })}><Plus size={17} /> Add record</button>} />}
      </section>
      {editing && <AdminEditor definition={definition} record={editing} onClose={() => setEditing(null)} onSave={save} />}
      {deleteCandidate && <AdminConfirm title={`Delete ${deleteCandidate[definition.titleField]}?`} copy="This removes the record from Supabase and the public collection. The action will be recorded in the audit trail." confirmLabel="Delete permanently" onCancel={() => setDeleteCandidate(null)} onConfirm={remove} />}
...
function AdminEditor({ definition, record, onClose, onSave }) {
  const initial = Object.fromEntries(definition.fields.map((field) => [field.key, record[field.key] ?? field.defaultValue ?? (field.type === 'boolean' ? false : '')]))
...
function AdminWorkflowPage() {
  ...
  return (
    <div className="admin-workflow-page">
      <AdminPageHeader eyebrow="Community operations" title={definition.label} copy={definition.description} actions={<button className="admin-secondary-action" onClick={load}><RefreshCw size={16} /> Refresh</button>} />
