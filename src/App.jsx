    ...
    <footer className="site-footer">
      <div className="shell shell--wide site-footer__top">
        <div className="footer-intro">
          <MalawiMark />
          <p>A living digital gateway to Malawiâ€™s landscapes, collections, stories and celebrations.</p>
        ...
        <div className="footer-links">
          ...
          <div><strong>Keep in touch</strong><a href="mailto:hello@tourismmalawi.mw">hello@tourismmalawi.mw</a><span>Lilongwe, Malawi</span><span>Monâ€“Fri Â· 08:00â€“17:00</span></div>
        </div>
      </div>
      <div className="shell shell--wide site-footer__bottom">
        <span>Â© 2026 Online Tourism Malawi</span>
  ...
  return (
    <>
      <section className="home-hero">
        ...
        <div className="shell shell--wide home-hero__content">
          <div className="home-hero__copy">
            <Eyebrow>Culture Â· Nature Â· Memory</Eyebrow>
            <h1>Malawi,<br /><em>alive in every story.</em></h1>
            <p>Step into the Warm Heart of Africaâ€”where ancient rock art, living traditions and unforgettable landscapes meet.</p>
      ...
      <section className="intro-band">
        <div className="shell intro-band__grid">
          ...
          <blockquote>â€œTakulandiraniâ€<small>Welcome, in Chichewa</small></blockquote>
      ...
      <section className="section section--ivory">
        <div className="shell">
          ...
          <div className="path-grid">
            {[
              { no: '01', title: 'Arts & Natural Heritage', copy: 'Rock art, mountains, wildlife and the lake that holds a nationâ€™s imagination.', image: images.mulanje, to: '/explore' },
              { no: '02', title: 'Museums & Collections', copy: 'Meet the objects and institutions safeguarding Malawiâ€™s many histories.', image: images.karonga, to: '/museums' },
              ...
              <Link className="path-card" to={item.to} key={item.title}>
      ...
      <section className="section section--dark">
        <div className="shell">
          <SectionHeading inverse eyebrow="Gather in Malawi" title={<>Whatâ€™s <em>happening</em></>} copy="Festivals, talks, performances and cultural encounters across the country." action={{ label: 'View all events', to: '/events' }} />
    ...
    <>
      ...
      <section className="section section--ivory">
        <div className="shell">
          ...
          <div className="result-note"><span>{String(filtered.length).padStart(2, '0')}</span> places to wander</div>
      ...
      <section className="map-feature">
        <div className="shell map-feature__inner">
...
function HeritageCard({ item, index }) {
  return (
    <article className={`heritage-card ${index === 0 ? 'heritage-card--large' : ''}`}>
      <Picture src={item.image} alt={item.title} />
...
function EmptyState() {
  return <div className="empty-state"><Search size={26} /><h3>No stories found</h3><p>Try a broader word or choose another category.</p></div>
}
  ...
  return (
    <>
      ...
      <section className="museum-intro section section--ivory">
        <div className="shell museum-intro__grid"><div><Eyebrow>More than objects</Eyebrow><h2>Every collection is<br /><em>a conversation.</em></h2></div><p>From fossil discoveries in Karonga to the expressive masks of Mua, Malawiâ€™s collections connect scientific discovery with lived cultural knowledge. Explore slowlyâ€”and listen to the voices around each object.</p></div>
      </section>
      <section id="museum-list" className="section section--paper">
        <div className="shell">
          ...
          <div className="museum-list">{museums.map((museum, index) => (
            <article className="museum-row" key={museum.title}>
      ...
      <section className="object-feature">
        <div className="shell object-feature__inner"><Picture src={images.gulePortrait} alt="Gule Wamkulu ceremonial attire" /><div><Eyebrow>Object stories</Eyebrow><span className="object-feature__tag">From the living collection</span><h2>Not simply<br /><em>a mask</em></h2><p>In Gule Wamkulu, costume, character, movement and audience are inseparable. Cultural meaning lives in performanceâ€”not only behind museum glass.</p><Link className="button button--outline" to="/performance">Meet the tradition <ArrowRight size={18} /></Link></div></div>
      </section>
    </>
  )
}
      ...
      <section className="section section--dark performance-browser">
        <div className="shell">
          ...
          <div className="performance-browser__grid">
            ...
            <article className="performance-focus">
...
function EventsPage() {
  ...
  return (
    <>
      <PageHero eyebrow="Come together" title={<>Events &<br /><em>Gatherings</em></>} copy="Find the festivals, performances, exhibitions and conversations bringing Malawiâ€™s cultural calendar to life." image={images.lakeSunset} imageAlt="Sunset gathering at Lake Malawi">
        <div className="hero-meta"><span><CalendarDays size={17} />September â€” November 2026</span><span><MapPin size={17} />Across Malawi</span></div>
      </PageHero>
      <section className="section section--ivory">
        <div className="shell">
          <div className="events-toolbar"><div className="filter-row" role="group" aria-label="Filter events">{types.map((type) => <button key={type} className={filter === type ? 'active' : ''} onClick={() => setFilter(type)}>{type}</button>)}</div><span>{visible.length} upcoming</span></div>
          <div className="events-page-grid">{visible.map((event) => <EventCard key={event.title} event={event} />)}</div>
        </div>
      </section>
      <section className="event-cta"><div className="shell event-cta__inner"><div><Eyebrow>Hosting something?</Eyebrow><h2>Put your event<br /><em>on the map.</em></h2></div><div><p>Community organisations, museums and cultural groups can share eligible public events with our editorial team.</p><a href="mailto:events@tourismmalawi.mw" className="button button--gold">Submit an event <ArrowRight size={18} /></a></div></div></section>
    </>
  )
}
    ...
    <>
      ...
      <section className="values-section">
        <div className="shell"><SectionHeading inverse eyebrow="What guides us" title="Four promises" />
          ...
          ].map(([no, title, copy]) => <article key={title}><span>{no}</span><Sparkles size={21} /><h3>{title}</h3><p>{copy}</p></article>)}</div>
        </div>
      </section>
      <section className="section section--paper story-timeline">
        <div className="shell story-timeline__grid"><Picture src={images.lake} alt="A view across Lake Malawi" /><div><Eyebrow>Our north star</Eyebrow><h2>A digital home<br /><em>with open doors.</em></h2><p>This first version is a foundation. The long-term vision is a platform where museums, artists, guides, scholars and communities can publish, teach and connect directly with audiences.</p><ul><li><Check size={17} />Museum and archive partnerships</li><li><Check size={17} />Community-led oral histories</li><li><Check size={17} />Accessible virtual exhibitions</li><li><Check size={17} />Responsible trip-planning tools</li></ul></div></div>
      </section>
      <Newsletter />
    </>
  )
...
function MediaPage() {
  ...
  return (
    <>
      <PageHero eyebrow="Watch Â· Listen Â· Remember" title={<>Media Library<br />& <em>Podcasts</em></>} copy="Let Malawi come closer through documentary shorts, field recordings, conversations and voices from the archive." image={images.ilala} imageAlt="Historic view of the Ilala on Lake Malawi">
        <div className="media-counts"><span><strong>42</strong> films</span><span><strong>68</strong> audio stories</span><span><strong>12</strong> collections</span></div>
      </PageHero>
      <section className="podcast-stage">
        <div className="shell">
          ...
          <div className="podcast-player">
            ...
            <div className="podcast-player__copy"><span>Episode {selected.number} Â· {selected.category}</span><h3>{selected.title}</h3><p>{selected.guest}</p><div className="wave" aria-hidden="true">{Array.from({ length: 42 }, (_, i) => <i key={i} style={{ height: `${12 + ((i * 19) % 42)}px` }} />)}</div><div className="player-controls"><button onClick={() => setIsPlaying((value) => !value)} aria-label={isPlaying ? 'Pause episode' : 'Play episode'}>{isPlaying ? <Pause fill="currentColor" /> : <Play fill="currentColor" />}</button><span>08:14</span><div className="progress"><i /></div><span>{selected.length}</span><Volume2 size={18} /></div></div>
          </div>
          <div className="episode-list">{podcasts.map((episode, index) => <button className={playing === index ? 'active' : ''} onClick={() => selectEpisode(index)} key={episode.title}><span>{episode.number}</span><Picture src={episode.image} alt="" /><div><strong>{episode.title}</strong><small>{episode.guest}</small></div><span>{episode.length}</span><i>{playing === index && isPlaying ? <Pause size={17} fill="currentColor" /> : <Play size={17} fill="currentColor" />}</i></button>)}</div>
        </div>
      </section>
      <section className="section section--ivory">
        <div className="shell"><SectionHeading eyebrow="From the library" title="Watch & discover" action={{ label: 'Browse all media', to: '/media-library' }} />
          ...
          ].map(([title, copy, image, time]) => <article key={title}><Picture src={image} alt={title} /><button aria-label={`Play ${title}`}><Play fill="currentColor" /></button><span>{time}</span><div><small>Documentary short</small><h3>{title}</h3><p>{copy}</p></div></article>)}</div>
        </div>
      </section>
      <section className="archive-banner"><div className="shell archive-banner__inner"><Mic2 size={34} /><div><Eyebrow>Contribute a memory</Eyebrow><h2>Your story belongs<br /><em>in the archive.</em></h2></div><div><p>Weâ€™re building a community audio collection. Tell us about a place, celebration, object or journey you want future generations to hear.</p><a href="mailto:stories@tourismmalawi.mw" className="button button--outline">Share your story <ArrowRight size={18} /></a></div></div></section>
    </>
  )
}
...
function SignInPage() {
  ...
  return (
    <section className="auth-page">
      ...
      <span className="auth-page__caption">Lake Malawi Â· The Lake of Stars</span>
    </section>
  )
}

function NotFoundPage() {
  return <section className="not-found"><div><span>404</span><Eyebrow>Off the map</Eyebrow><h1>This trail ends here.</h1><p>The page youâ€™re looking for may have moved, but there is plenty more Malawi to discover.</p><Link className="button button--gold" to="/">Return home <ArrowRight size={18} /></Link></div></section>
}
